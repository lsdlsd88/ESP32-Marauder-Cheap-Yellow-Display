...
/***************************************************************************************
** Function name:           validTouchBBC
** Description:             read validated position. Return false if not pressed. 
***************************************************************************************/
#define _RAWERR 20 // Deadband error allowed in successive position samples
uint8_t TFT_eSPI::validTouchBBC(uint16_t *x, uint16_t *y, uint16_t threshold){
  uint16_t x_tmp, y_tmp, x_tmp2, y_tmp2;


  bool touched=getTouchBBC(&x_tmp,&y_tmp);
...
/***************************************************************************************
** Function name:           getTouchBBC
** Description:             read calibrated position. Return false if not pressed. uses BB_captouch library 
***************************************************************************************/
uint8_t TFT_eSPI::getTouchBBC(uint16_t *x, uint16_t *y, uint16_t threshold){
  uint16_t x_tmp, y_tmp;
...
bool touched=false;
  ...
  if (bbct.getSamples(&ti)) {
      ...
      Serial.print("  x: ");Serial.print(ti.x[0]);x_tmp=ti.x[0];
      Serial.print("  y: ");Serial.print(ti.y[0]);y_tmp=ti.y[0];
      Serial.println(' ');
      touched=true;
  Serial.println(touched);
...
/***************************************************************************************
** Function name:           calibrateTouchBBC
** Description:             generates calibration parameters for touchscreen. 
***************************************************************************************/
void TFT_eSPI::calibrateTouchBBC(uint16_t *parameters, uint32_t color_fg, uint32_t color_bg, uint8_t size){
  int16_t values[] = {0,0,0,0,0,0,0,0};
  uint16_t x_tmp, y_tmp;
  ...
  for(uint8_t i = 0; i<4; i++){
    ...
    for(uint8_t j= 0; j<8; j++){
      // Use a lower detect threshold as corners tend to be less sensitive
      while(!validTouchBBC(&x_tmp, &y_tmp, Z_THRESHOLD/2));
      values[i*2  ] += x_tmp;
      values[i*2+1] += y_tmp;
      }
    values[i*2  ] /= 8;
